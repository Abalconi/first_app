...
st.title(f"Dashboard de Ventas", anchor=False)
...
analysis_type = left_col.selectbox(
    label="Análisis por:",
    options=["Mes", "Categoria de Productos"],
    key="analysis_type",
)
selected_city = right_col.selectbox("Selecciona una Ciudad:", CITIES)

# Toggle for selecting the year for visualization
previous_year_toggle = st.toggle(
    value=False, label="Año Anterior", key="switch_visualization"
)
...
st.write(f"**Ventas por {visualization_year}**")

# Filter data based on selection for visualization
if analysis_type == "Categoria de Productos":
    filtered_data = (
